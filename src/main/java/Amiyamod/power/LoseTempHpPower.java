...
public class LoseTempHpPower extends AbstractPower implements OnLoseTempHpPower {
    public static final String NAME = "LoseTempHpPower";
    public static final String POWER_ID = Amiyamod.makeID(NAME);
    private static final PowerStrings powerStrings = CardCrawlGame.languagePack.getPowerStrings(POWER_ID);
    public static final String[] DESCRIPTIONS = powerStrings.DESCRIPTIONS;
    public LoseTempHpPower() {
    ...
    // 效果 :失去生命后触发卡牌 OnLoseTempHpPower
        ...
        if ( damageAmount > 0 && tem >= damageAmount){
            for (AbstractCard card : AbstractDungeon.player.hand.group){
                if (card instanceof OnLoseTempHpPower){
                    ((OnLoseTempHpPower)card).onLoseTempHp(new DamageInfo(this.owner,damageAmount), damageAmount);
                }
            }
