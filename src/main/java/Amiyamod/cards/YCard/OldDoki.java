...
public class OldDoki extends CustomCard {
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        List<AbstractCard> list = CardLibrary.getCardList(LibraryTypeEnum.AMIYA);
        CardGroup l2 = new CardGroup(CardGroup.CardGroupType.UNSPECIFIED);
        for (AbstractCard card : list){
            if (card.hasTag(YCardTagClassEnum.YCard)&&!card.hasTag(CardTags.HEALING)){
                l2.addToBottom(card);
            }
        }
        boolean dupe = false;
